...
#
#-------------------------------------------------------------------------
from gramps.gen.plug.docgen import TreeDoc
from gramps.gen.utils.file import search_for, media_path_full
from gramps.gen.lib import Person, EventType, EventRoleType, Date
from gramps.gen.display.place import displayer as _pd
from gramps.gen.plug.menu import NumberOption, TextOption, EnumeratedListOption
from gramps.gen.constfunc import win
from gramps.gen.config import config
from gramps.gen.const import GRAMPS_LOCALE as glocale
from .latexbasedoc import LaTeXBaseDoc, LaTeXBaseDocOptions, escape
...
#
#------------------------------------------------------------------------------
class LaTeXTreeOptions(LaTeXBaseDocOptions):
    """
...
#
#------------------------------------------------------------------------------
class LaTeXTreeDocBase(LaTeXBaseDoc, TreeDoc):
    """
    ...
    def __init__(self, options, paper_style):
        LaTeXBaseDoc.__init__(self, None, paper_style, options, None)
    ...
    def write_start(self):
        ...
        if self.nodecolor == 'preferences':
            ...
            self.emit('\\definecolor{male-bg}{HTML}{%s}\n' % male_bg)
            self.emit('\\definecolor{female-bg}{HTML}{%s}\n' % female_bg)
            self.emit('\\definecolor{neuter-bg}{HTML}{%s}\n' % neuter_bg)
        ...
        self.emit('\\begin{tikzpicture}\n')

    def start_tree(self, option_list):
        self.emit('\\genealogytree[\n')
        self.emit('processing=database,\n')
        if self.marriage:
            info = self.detail + ' ' + self.marriage
        else:
            info = self.detail
        self.emit('database format=%s,\n' % info)
        if self.timeflow:
            self.emit('timeflow=%s,\n' % self.timeflow)
        if self.edges:
            self.emit('edges=%s,\n' % self.edges)
        if self.leveldist != 5:
            self.emit('level distance=%smm,\n' % self.leveldist)
        if self.nodesize != 25:
            self.emit('node size=%smm,\n' % self.nodesize)
        if self.levelsize != 35:
            self.emit('level size=%smm,\n' % self.levelsize)
        if self.nodecolor == 'none':
            self.emit('tcbset={male/.style={},\n')
            self.emit('        female/.style={},\n')
            self.emit('        neuter/.style={}},\n')
        if self.nodecolor == 'preferences':
            self.emit('tcbset={male/.style={colback=male-bg},\n')
            self.emit('        female/.style={colback=female-bg},\n')
            self.emit('        neuter/.style={colback=neuter-bg}},\n')
    ...
    def start_subgraph(self, level, subgraph_type, family, option_list=None):
        ...
        self.emit('%s[%s]{\n' % (subgraph_type, ','.join(options)), level)
    ...
    def write_node(self, db, level, node_type, person, marriage_flag,
        ...
        self.write_node_name(level+1,first,call,surn,nick)
    ...
    def write_event(self, db, level, event):
        ...
        if modifier:
            event_type += '+'
            self.emit('%s = {%s}{%s}{%s},\n' %
                       (event_type, date_str, place, modifier), level)
        elif place == '':
            event_type += '-'
            self.emit('%s = {%s},\n' % (event_type, date_str), level)
        else:
    ...
    def format_iso(self, date_tuple, calendar):
        ...
        return iso_date
         ...
         # TreeDoc interface
    def write_node_name(self, level, firstnames, callname, surname, nickname):

        def format_given_names(firstnames, callname):
            first = firstnames
            call = callname
            if call:
                if call in first:
        ...
        name_parts = [format_given_names(firstnames, callname),
                      '\\nick{{{}}}'.format(escape(nickname)) if nickname else '',
                      '\\surn{{{}}}'.format(escape(surname)) if surname else '']
        self.emit('name = {{{}}},\n'.format(
            ' '.join([e for e in name_parts if e])),level)
    def open(self, filename):
        self._filename = os.path.normpath(os.path.abspath(filename))
        LaTeXBaseDoc.open(self, self._filename)
        self.write_start()


    def close(self):
        self.write_end()
        LaTeXBaseDoc.close(self)
...
#
#------------------------------------------------------------------------------
class TreeTexDoc(LaTeXTreeDocBase):
    """
...
#
#------------------------------------------------------------------------------
class TreePdfDoc(LaTeXTreeDocBase):
    """
